...
function BFS(from, to, vertical, nolimit)
	print("RECALCULATING BFS")

	local searchingDistance = 200 -- BFS depth
	local queue,l,r, dist, tmp
	...
	answ = nil
	answdist = 10000000000

	dist={}
	tmp=-1
	Tset(dist, tmp, from.x, from.y, from.z)

	while (l<r) and (l<searchingDistance) do
		local act, continue
		continue=true
		...
		if (continue) then --IF CAN MOVE THROUGHT THEN OK

			local distan = distance(act,to)
			...
			if (r < searchingDistance) then

				if (vertical) then  -- FIRST PRIORITY - MOVE DOWN
					local f=5
					local actf={}
					actf.x = act.x
					actf.y = act.y - 1
					actf.z = act.z
					parb = Tget(dist, actf.x, actf.y, actf.z)
...
function turtleUpdate()  -- Updating breakable block database - maybe turtle or tree ?
	while (true) do
		ev = { os.pullEventRaw() }
		-- Check something
			if (ev[1]=="timer")then
...
function CoroutineControl.addCoroutine(self,func,Arg)
	...
	if (Arg) then  -- ADD ARG
		self.coroutines[id].ok, self.coroutines[id].requestedEvent = coroutine.resume(self.coroutines[id].cID, unpack( Arg ))
	else
		self.coroutines[id].ok, self.coroutines[id].requestedEvent = coroutine.resume(self.coroutines[id].cID)
	end
	return id
end
		...
		for id,coro in pairs(self.coroutines) do
			if (coro.run) then
				if (ev[1]==coro.requestedEvent or coro.requestedEvent==nil) then
					coro.ok, coro.requestedEvent = coroutine.resume(coro.cID,unpack( ev ))
				end
			end
		end
	end
